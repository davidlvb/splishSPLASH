...
viewMatrix <- function(input, output, session, refGenome, parsedData){
  ...
  ### Execute blob auto-finding upon click ###
  observeEvent(input$autoFind_run, {
    ...
    withProgress(min=0, max=numPairs, message = "Finding local maxima",{
      ...
      for (i in 1:numPairs){
        ...
        matMaxima <- findLocalMaxima_cpp(mat, input$findMaxima_baseline, input$findMaxima_searchBox, input$findMaxima_dilate)
        if (input$findMaxima_grow > 0){
          matMaxima <- as.matrix(grow(as.pixset(as.cimg(matMaxima)), input$findMaxima_grow))
        }
        if (input$findMaxima_shrink > 0){
          matMaxima <- as.matrix(shrink(as.pixset(as.cimg(matMaxima)), input$findMaxima_shrink))
        }
        blobs <- findBlobs(dfParsed, matMaxima)
        if (input$findMaxima_minArea > 0){
          blobs <- dplyr::filter(blobs, npixel >= input$findMaxima_minArea)
        }
        
        # blobMask <- (grow(as.pixset(as.cimg( findLocalMaxima_cpp(mat, input$findMaxima_baseline, input$findMaxima_searchBox, input$findMaxima_dilate))), input$findMaxima_grow))
        # # Run blob-finding algorithm
        # blobs <- findBlobs(dfParsed, blobMask)
        if(nrow(blobs) == 0) { toc(quiet=TRUE); cat('no blobs\n'); next }
    ...
    content = function(con) {
      withProgress(message="Gathering data", detail="... this may take a while", {
        ...
        if(input$autoFind_downloadTableOutput_classic == TRUE){
          tableToWrite <- dplyr::select(tableToWrite, seg1, minX, maxX, seg2, minY, maxY, readsBounded)
